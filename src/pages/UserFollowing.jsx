...
const UserFollowing = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setMessage('');
        const [summaryRes, followingRes] = await Promise.all([
        ...
        setMessage(err.response?.data?.message || 'Erro ao carregar seguindo.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleToggleFollow = async (userId) => {
    ...
    try {
      ...
      const nowFollowing = response.data.following;

      setFollowing((prev) => {
        if (isMe && !nowFollowing) return prev.filter((u) => u.id !== userId);
        return prev.map((u) => (u.id === userId ? { ...u, isFollowing: nowFollowing } : u));
      });

      if (isMe) {
        setSummary((prev) =>
          prev ? { ...prev, followingCount: Math.max(0, (prev.followingCount || 0) + (nowFollowing ? 1 : -1)) } : prev
        );
      }
    } catch (err) {
  ...
  if (loading) return <div className="text-center py-12 text-slate-400 font-bold animate-pulse">Carregando seguindo...</div>;
      ...
      <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Link to={`/user/${id}/followers`} className="flex items-center px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-xs font-black uppercase tracking-widest border border-slate-100 hover:bg-slate-100 transition-colors">
              <Users size={14} className="mr-2" /> {summary?.followersCount || 0} seguidores
            </Link>
            <Link to={`/user/${id}/following`} className="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest border border-indigo-600">
      ...
      <div className="space-y-4">
        {following.length > 0 ? (
          ...
          <div className="p-16 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200 text-center space-y-3">
            <p className="text-lg font-bold text-slate-400 italic">Ainda não está seguindo ninguém.</p>
            <p className="text-sm text-slate-300 font-medium">Siga estudantes para ver o feed de “Seguindo”.</p>
          </div>
        )}
      </div>
    </div>
